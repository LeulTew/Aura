...
const GalleryView = forwardRef<GalleryViewHandle, GalleryViewProps>(
  ({ matches, onBack, isBundle = false, hideNavActions = false }, ref) => {
    ...
    // Generate signed URLs for all images on mount
    useEffect(() => {
      const generateSignedUrls = async () => {
        ...
        for (const match of matches) {
          const { data } = await supabase.storage
            .from('photos')
            .createSignedUrl(match.source_path, 3600); // 1 hour expiry
          if (data?.signedUrl) {
            urlMap.set(match.source_path, data.signedUrl);
          }
        }
    ...
    const getImageUrl = (sourcePath: string) => {
      return signedUrls.get(sourcePath) || '';
    };
      ...
      <div className="min-h-screen bg-[var(--bg)] overflow-x-hidden">
        ...
        {!hideNavActions && selectMode && selected.size > 0 && !viewerOpen && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-6 px-8 py-4 bg-[rgba(10,10,10,0.8)] backdrop-blur-2xl border border-white/10 rounded-full z-[100] animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <button
              onClick={downloadAll}
              className="text-[var(--accent)] font-mono text-xs font-bold uppercase tracking-[0.2em] hover:text-white transition-colors"
            >
              Download
            </button>
          </div>
